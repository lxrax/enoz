...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO locational_clearances (
                    ...
                    condition9_revoked, condition10_provisional, applicant_address, address, issue_date, owner_name, project_type
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            mysqli_stmt_bind_param($stmt, "sssssssssssssdsiiiiiiiiiisssss",
                $applicant_name, $developer_name, $developer_address, $project_location, $date_filed, $expiration_date, $clearance_number,
                $project_name, $right_over_land, $land_area, $building_area, $decision,
                $or_number, $amount_paid, $encoded_by_user_id,
                ...
                $conditions['condition9'], $conditions['condition10'], $applicant_address, $applicant_address, $issue_date, $developer_name, $project_name
            );
